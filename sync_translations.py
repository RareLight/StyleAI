...
def extract_loc_keys(directory):
    ...
    # Match the delimiter that opened the Lua string. Apostrophes are valid in
    # double-quoted defaults (and double quotes in single-quoted defaults), so
    # treating either quote as a terminator truncates otherwise valid strings.
    pattern = re.compile(
        r"""LOC\s*\(?\s*(?:"(\$\$\$/StyleAI/(?:\\.|[^"\\])*)"|'(\$\$\$/StyleAI/(?:\\.|[^'\\])*)')"""
    )
            ...
            if file.endswith(".lua"):
                ...
                with open(path, "r", encoding="utf-8", errors="ignore") as f:
                    ...
                    for double_quoted, single_quoted in matches:
                        m = double_quoted or single_quoted
                        if "=" in m:
                            key, val = m.split("=", 1)
                            keys[key] = val
                        else:
...
def sync_translations(lua_dir, target_path, base_strings=None):
    ...
    for key in keys_to_use:
        ...
        if not key.startswith("$$$/StyleAI/"):
            if key not in extracted_keys and (
                not base_strings or key not in base_strings
            ):
                continue
        ...
        if not base_strings:
            ...
            # Existing translations are authoritative. New keys temporarily
            # fall back to English until a translator supplies the locale value.
            if not val or val == "":
