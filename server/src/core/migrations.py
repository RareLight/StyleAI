...
def run_migrations(db_path: str):
    ...
    applied_versions = {
        row[0] for row in conn.execute("SELECT version_id FROM schema_versions").fetchall()
    }
    ...
    for filename in migration_files:
        ...
        spec = importlib.util.spec_from_file_location(f"migration_{version_id}", filepath)
        ...
        if hasattr(migration_module, "upgrade"):
            ...
            logger.warning(f"Migration {version_id} has no 'upgrade' function. Skipping.")
