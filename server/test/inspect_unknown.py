...
def inspect():
    try:
        client = chromadb.PersistentClient(path="/Users/anna/Pictures/Lightroom Classic/Rare Light Photography/styleai.db")
        ...
        for k, v in sorted(keywords_tally.items(), key=lambda x: x[1], reverse=True)[:10]:
            print(f"  - {k}: {v}")
            
        print("\nTop AI scene tags for 'Unknown':")
        for k, v in sorted(scene_tags_tally.items(), key=lambda x: x[1], reverse=True)[:10]:
            print(f"  - {k}: {v}")
