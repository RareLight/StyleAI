...
- **Bounded pipeline**: The server owns job admission, image-byte budgets, per-item completion state, and cancellation. Lua must not create unbounded producer queues or multiple long-running LLM requests. Metadata generation may begin only after the corresponding embedding phase has committed successfully; a combined-workflow item remains nonterminal until metadata and any immediate Lightroom catalog handoff finish.
- **Durable operation jobs**: Indexing, tagging, training/discovery, and editing must use catalog-local operation jobs with per-photo terminal states. Backend success that still requires a Lightroom catalog/apply handoff uses a nonterminal state; Lightroom records the final result. Cancellation is scoped to one job; global cancellation is reserved for backend shutdown. Multiple Lightroom tasks share process-wide admission rather than multiplying concurrency.
- **Resource vectors**: Acquire every required backend resource atomically through `services.operations.admission`. Accelerator and local-LLM capacity are process-wide; training uploads queue as complete workflows; catalog writes are short critical sections. Lightroom export, catalog-write, Develop/UI, and backend-request lanes use `WorkCoordinator.lua`.
...
- **Runtime pressure**: Hardware tiers are startup maxima, not throughput targets that must always be filled. Use the operations pressure governor for effective CPU, GPU-batch, and image-byte limits; it may scale limits downward under memory pressure but never above detected or explicitly configured maxima.
...
- **Maintenance isolation**: Backups, pruning, destructive resets, and policy activation share the maintenance/catalog-write admission lanes. A pre-prune backup must be durably copied before deletion begins. Lifecycle marker cleanup must verify process ownership so an exiting backend cannot remove a replacement process's PID/OK files.
...
- **No fixed zero-shot scene probes**: Do not assign training labels or index metadata from a built-in list of scene/genre text prompts. User-authored open-vocabulary keywords may explain policies after discovery; locally generated descriptors remain provenance-labelled diagnostics and never determine membership.
