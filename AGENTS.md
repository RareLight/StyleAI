...
- **DO NOT Hard-Fail Metadata Generation when Image Pixels are Absent**: If users execute "LLM-only" batch generation (e.g. re-using existing vision tags to generate descriptions), the plugin intentionally skips exporting JPEGs to the backend cache to save time. The backend's `/metadata/generate_batch` and `/metadata/generate` endpoints MUST handle missing `image_bytes` gracefully (proceeding to generate text-only metadata) instead of returning HTTP 400 errors or skipping tasks.
...
- **DO NOT Nest Database Transactions Inside Iteration Loops**: When executing Lightroom batch operations (like creating multiple collections or assigning metadata), DO NOT place `catalog:withWriteAccessDo` inside a `for` loop. Opening and closing hundreds of SQLite transactions sequentially causes severe UI thread blocking and transaction bloat. Always consolidate batch operations into a single `withWriteAccessDo` block that wraps the entire iteration loop.
...
- **DO NOT Query Children of Newly Created Collections**: When batch-creating collections, calling `getChildCollections()` on a newly created `LrCollectionSet` within the SAME `withWriteAccessDo` transaction throws a fatal internal SDK error. Always track newly created sets in memory and skip querying their children until the transaction commits.
- **DO Synchronize Dependent GPU Worker Pipelines**: When chaining asynchronous background workers (e.g., Vision Embedding -> LLM Metadata generation), do not allow the downstream worker to consume the queue if the upstream worker has not yet committed its outputs (embeddings) to the database. Doing so causes the downstream worker to fail its lookup and fallback to slow, unbatched sequential re-computation, destroying GPU throughput. Always use a synchronization gate (e.g., `active_embeddings_uuids`) to pause downstream consumers until the batch dependency is fully resolved.
